...
const Verify = () => {
  ...
  const cities = [
    ...
    "Philadelphia", "San Antonio", "San Diego", "Dallas", "San Jose", "Vijayawada"
  ];
  ...
  const extractTransactionHash = (qrData: string): string | null => {
    try {
      // Try to parse as JSON first
      try {
        const parsedData = JSON.parse(qrData);
        if (parsedData.transaction_hash) {
          return parsedData.transaction_hash;
        }
        if (parsedData.transactionHash) {
          return parsedData.transactionHash;
        }
      } catch (e) {
        // Not JSON, continue with other methods
      }
      
      // Check if it's a direct transaction hash (64 hex characters)
      if (qrData.length === 64 && /^[0-9a-fA-F]+$/.test(qrData)) {
        return qrData;
      }
      
      // Check if it's an explorer URL
      if (qrData.includes("explorer.aptoslabs.com")) {
        const url = new URL(qrData);
        const pathParts = url.pathname.split("/");
        const hashIndex = pathParts.indexOf("txn");
        if (hashIndex !== -1 && hashIndex + 1 < pathParts.length) {
          return pathParts[hashIndex + 1];
        }
        
        // Alternative URL pattern matching
        const match = qrData.match(/txn\/([0-9a-fA-F]{64})/);
        if (match && match[1]) {
          return match[1];
        }
      }
  ...
  const fetchItemDetails = async (transactionHash: string) => {
    try {
      // First, get all items for the module
      ...
      if (response && response[0] && Array.isArray(response[0])) {
        // Find the item with the matching transaction hash
        const items: Item[] = response[0].map((item: any) => ({
        ...
        const foundItem = items.find(item => item.transaction_hash === transactionHash);
        ...
        if (foundItem) {
          ...
          setVerificationMessage("No item found with this transaction hash.");
          return null;
        }
      }
    } catch (err) {
      ...
      setVerificationResult(false);
      setVerificationMessage("Error fetching item details from blockchain.");
      return null;
    }
  };
  ...
  const handleScanResult = async (result: any, error: any) => {
    if (result) {
      try {
        ...
        const transactionHash = extractTransactionHash(result.text);
        
        if (transactionHash) {
          setIsScanning(false);
          setIsProcessing(true);
          const item = await fetchItemDetails(transactionHash);
          ...
          setVerificationResult(false);
          setVerificationMessage("Invalid QR code format. Could not extract transaction hash.");
        }
      } catch (e) {
        console.error("Error processing QR code data:", e);
        setVerificationResult(false);
        setVerificationMessage("Error processing QR code data.");
      }
    }
  ...
  const handleFileUpload = (file: File) => {
    ...
    reader.onload = async (e) => {
      ...
      image.onload = () => {
        ...
        if (!ctx) {
          setVerificationResult(false);
          setVerificationMessage("Could not process image.");
          setIsProcessing(false);
          return;
        }
        ...
        try {
          ...
          if (code) {
            ...
            const transactionHash = extractTransactionHash(code.data);
            if (transactionHash) {
              fetchItemDetails(transactionHash).then(item => {
                setIsProcessing(false);
                if (item) {
                  setScannedData({
                    itemType: item.item_type,
            ...
            setVerificationResult(false);
            setVerificationMessage("No QR code found in the image.");
            setIsProcessing(false);
          }
        } catch (error) {
          console.error("Error decoding QR code:", error);
          setVerificationResult(false);
          setVerificationMessage("Error decoding QR code from image.");
          setIsProcessing(false);
        }
      };
      image.src = imageData;
  ...
  const verifyOwnership = async () => {
    if (!scannedData || !selectedCity) {
      setVerificationResult(false);
      setVerificationMessage("Please select a city and ensure QR code is scanned correctly.");
      return;
    }
    ...
    try {
      if (walletAddress.toLowerCase() === scannedData.walletAddress.toLowerCase()) {
        setVerificationResult(true);
        setVerificationMessage("Verification successful! Wallet address matches.");
      } else {
        setVerificationResult(false);
        setVerificationMessage("Verification failed. Wallet address does not match.");
      }
    } catch (error) {
      setVerificationResult(false);
      setVerificationMessage("Error during verification process.");
    } finally {
        ...
        <div className="flex space-x-8">
          {["Home", "Admin", "My Orders", "Verify", "Learn"].map((tab) => (
            ...
            >
              {tab}
              <span
                className={`absolute bottom-0 left-0 w-full h-0.5 bg-orange-600 transform origin-left transition-transform duration-300 ${tab === "Verify" ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
              ></span>
            </button>
          ))}
        </div>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-sm border border-gray-100">
          ...
          <p className="text-gray-600 text-center mb-10">
            Scan the QR code to verify product details and authenticity on the blockchain
          </p>
          ...
          {!isConnected ? (
            ...
            <>
              {!scannedData && verificationResult === null && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                  ...
                  >
                    ...
                    {cameraPermission === false && <p className="text-xs text-red-500 mt-2">Camera access required</p>}
                  </div>
                  ...
                  >
                    ...
                    {isDragOver && <p className="text-xs text-orange-500 mt-2">Drop image here</p>}
                  </div>
                </div>
              )}
              ...
              {scanMethod === "upload" && !scannedData && verificationResult === null && (
                <div
                ...
                >
                  ...
                  >
                    <path
                      ...
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M7 16a4 4 0 01-.88-7.903A5 5 0 11115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"
                    />
                  </svg>
              ...
              {scannedData && verificationResult === null && (
                <div className="mb-10">
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    <p className="text-blue-800 font-medium">QR code scanned successfully!</p>
                    <p className="text-blue-700 text-sm mt-1">
                      Transaction hash: {scannedData.walletAddress.substring(0, 12)}...
                      {scannedData.walletAddress.substring(scannedData.walletAddress.length - 6)}
                    </p>
                  </div>
                  ...
                  <div className="border border-gray-200 rounded-lg p-6 mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="font-medium text-sm">{shortenAddress(scannedData.walletAddress)}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">
                          {scannedData.pickupDate} at {scannedData.pickupTime}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Select Verification City</label>
                    <select
                      value={selectedCity}
                  ...
                  <button
                    ...
                    disabled={!selectedCity || isVerifying}
                    className="w-full py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg hover:from-orange-600 hover:to-orange-700 transition-all duration-300 disabled:opacity-75 flex items-center justify-center font-medium shadow-md hover:shadow-lg"
                  >
                    {isVerifying ? (
                      <>
                        <svg
                    ...
                    <div className="w-64 h-64 border-4 border-dashed border-gray-300 rounded-lg flex items-center justify-center">
                      {verificationResult ? (
                        <div className="text-center">
                          ...
                          <p className="text-green-600 text-sm mt-2">{verificationMessage}</p>
                        </div>
                      ) : (
                        <div className="text-center">
                          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-4">
                          ...
                          <p className="text-red-600 text-sm mt-2">{verificationMessage}</p>
                        </div>
                      )}
                    </div>
                  </div>
